...
class SolverWrapper(object):
	...
	def construct_graph(self):
		...
		self.net.create_architecture(self.imdb.num_classes, tag='default', anchor_scales=cfg.ANCHOR_SCALES,
	...
	def find_previous(self):
		...
		for stepsize in cfg.TRAIN.STEPSIZE:
			redfiles.append(
				os.path.join(self.output_dir, cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_{:d}.pth'.format(stepsize + 1)))
	...
	def train_model(self, max_iters):
		...
		while iter < max_iters + 1:
			...
			moving_total_loss = moving_total_loss * 0.9 + total_loss * 0.1
			# Display training information
			if iter % (cfg.TRAIN.DISPLAY) == 0:
				print('iter: %d / %d, total loss: %.6f (%.6f)\n >>> rpn_loss_cls: %.6f\n '
				      '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> lr: %f' % (
				      iter, max_iters, total_loss, moving_total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, lr))
				print('speed: {:.3f}s / iter'.format(utils.timer.timer.average_time()))

				# for k in utils.timer.timer._average_time.keys():  #   print(k, utils.timer.timer.average_time(k))
